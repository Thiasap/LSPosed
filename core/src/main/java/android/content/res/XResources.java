...
import static de.robv.android.xframe.XframeHelpers.decrementMethodDepth;
import static de.robv.android.xframe.XframeHelpers.findAndHookMethod;
import static de.robv.android.xframe.XframeHelpers.getBooleanField;
import static de.robv.android.xframe.XframeHelpers.getLongField;
import static de.robv.android.xframe.XframeHelpers.getObjectField;
import static de.robv.android.xframe.XframeHelpers.incrementMethodDepth;
...
import de.robv.android.xframe.IXframeHookZygoteInit;
import de.robv.android.xframe.XC_MethodHook;
import de.robv.android.xframe.XC_MethodHook.MethodHookParam;
import de.robv.android.xframe.XframeBridge;
import de.robv.android.xframe.XframeBridge.CopyOnWriteSortedSet;
import de.robv.android.xframe.XframeInit;
import de.robv.android.xframe.callbacks.XC_LayoutInflated;
import de.robv.android.xframe.callbacks.XC_LayoutInflated.LayoutInflatedParam;
import de.robv.android.xframe.callbacks.XCallback;
import xframe.dummy.XResourcesSuperClass;
import xframe.dummy.XTypedArraySuperClass;
...
//			Log.w(XposedBridge.TAG, "Package name for " + resDir + " had to be retrieved via parser");
	 ...
	 * {@link IXframeHookZygoteInit#initZygote initZygote()} for all apps. As there is no
			...
			if (incrementMethodDepth("getDrawable") == 1) {
				...
				if (replacement instanceof DrawableLoader) {
					try {
						...
						if (result != null)
							return result;
					} catch (Throwable t) { XframeBridge.log(t); }
				} else if (replacement instanceof Integer) {
			...
			if (callbacks != null) {
				...
				if (value.type == TypedValue.TYPE_STRING) {
					...
					if (components.length == 3)
						...
						XframeBridge.log("Unexpected resource path \"" + value.string.toString()
								+ "\" for resource id 0x" + Integer.toHexString(id));
				} else {
					XframeBridge.log(new NotFoundException("Could not find file name for resource id 0x") + Integer.toHexString(id));
				}
	...
	public void getValue(int id, TypedValue outValue, boolean resolveRefs) throws NotFoundException {
		...
		if (replacement instanceof XResForwarder) {
			...
			if (replacement != null) {
				XframeBridge.log("Replacement of resource ID #0x" + Integer.toHexString(id) + " escaped because of deprecated replacement. Please use XResForwarder instead.");
			}
	...
	private static int translateResId(int id, XResources origRes, Resources repRes) {
		try {
			...
			if (!repResDefined && origResId == 0 && !entryType.equals("id")) {
				XframeBridge.log(entryType + "/" + entryName + " is neither defined in module nor in original resources");
				return 0;
			}
	...
	private static int translateAttrId(String attrName, XResources origRes) {
		...
		try {
			...
			XframeBridge.log("Attribute " + attrName + " not found in original resources");
		}
		return origAttrId;
	}
